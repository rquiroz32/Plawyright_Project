...
const testData = require('../../testData/ts2_tc2.4_filterBySeverity_testData.json')

//Data Driven tests based on '../../testData/ts2_tc2.4_filterBySeverity_testData.json' 
test.describe('Test Cases for TS2 Check Filter Functionality, TC2.4 Filter By Severity"', () => {
    testData.forEach(data => {
        test(`Data driven tests for ${data.testCase_Title}`, async ({ page }) => {
            const reportsPage = new ReportsPage(page)
            await reportsPage.goto()
            if (data.FilterValue === "Severity") {
                await reportsPage.filterBySeverityDropdown.click();
