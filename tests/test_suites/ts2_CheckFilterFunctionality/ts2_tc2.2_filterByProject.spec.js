...
const testData = require('../../testData/ts2_tc2.2_filterByProject_testData.json')

//Data Driven tests based on '../../testData/ts2_tc2.2_filterByProject_testData.json' 
test.describe('Test Cases for TS2 Check Filter Functionality, TC2.2 Filter By Project"', () => {
    testData.forEach(data => {
        test(`Data driven tests for ${data.testCase_Title}`, async ({ page }) => {
            const reportsPage = new ReportsPage(page)
            await reportsPage.goto()
            if (data.FilterValue === "Project") {
                await reportsPage.filterByProjectDropdown.click();
